...
void draw(void)
{
    // iterate over grid and print chars
    for (int row = 0; row < DIMENSION; row++)
    {
...
bool lookup(string s)
{
    // the dictonary is sorted alphabetically so we can use binary search
    int last = WORDS - 1, first = 0, middle;
    ...
    while (last >= first)
    {
        ...
        if (strcmp(dictionary.words[middle].letters, s) == 0 && (dictionary.words[middle].found == false))
        {
            dictionary.words[middle].found = true;
            return true;
        }
        else if (strcmp(dictionary.words[middle].letters, s) > 0)
        {
            last = middle - 1;
        }
        else
...
void scramble(void)
{
    char grid_clone[DIMENSION][DIMENSION];
    for (int row = 0; row < DIMENSION; row++)
    {
        for (int col = 0; col < DIMENSION; col++)
        {
            grid_clone[row][col] = grid[row][col];
        }
    }
    ...
    for (int row = 0; row < DIMENSION; row++)
    {
        for (int col = 0; col < DIMENSION; col++)
        {
            grid[col][DIMENSION - (row + 1)] = grid_clone[row][col];
        }
    }
}
